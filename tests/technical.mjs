...
assert.match(a2Source, /validateA2Contracts\(state\.data, state\.matrix\)/, "A2 validates the matrix against the published catalog at runtime");
assert.match(a2Source, /function renderSequentialInteraction\(exercise, attempt\)/, "A2 multi-turn tasks reveal partner turns sequentially");
assert.match(a2Source, /interactionTurns\.slice\(0, step \+ 1\)/, "future partner turns stay hidden until the current response is saved");
assert.match(a2Source, /speakFrenchSequence\(exercise\.audioScene/, "A2 can play a scene with per-turn voices");
...
assert.doesNotMatch(hiddenTargetLab, /Réponse modèle/, "a hidden oral model is not embedded in the DOM before reveal");
...
assert.match(a2Source, /async function markLessonComplete[\s\S]*?if \(!\(await saveA2State\(\)\)\) \{[\s\S]*?completedLessons = previousCompletedLessons/, "A2 rolls lesson completion back when persistence fails");
assert.match(a2Source, /async function toggleLessonComplete[\s\S]*?if \(!\(await saveA2State\(\)\)\) \{[\s\S]*?completedLessons = previousCompletedLessons/, "A2 rolls tile changes back when persistence fails");
