...
import { checkExercise, normalizeAnswer, stripDiacritics } from "../../exercises.js";
import { validateCourseCatalog } from "../../course-validator.js";
import { createCourseRuntime } from "../../course-runtime.js";
import {
...
const state = {
  ...
  listeningLadder: defaultListeningLadder(),
  ...
  pronunciationReviewQueueOrder: null,
...
const { evaluateLessonReadiness, getCompletedLessonIds } = mastery;
const courseRuntime = createCourseRuntime({
  ...
  renderers: {
    ...
    listening: renderListeningLadder,
  ...
  beforeSwitch: (_nextView, previousView) => {
    stopRecording();
    clearReviewRefreshTimer();
    state.appState.scrollPositions[previousView] = window.scrollY;
    state.reviewSeen.clear();
    state.reviewQueueOrder = null;
    state.reviewAnswerVisible = false;
    state.pronunciationReviewSeen.clear();
    state.pronunciationReviewQueueOrder = null;
    state.pronunciationReviewAnswerVisible = false;
  },
  saveView: () => saveAppState(),
  beforeRender: () => releaseAudioUrls(),
  afterRender: () => {
...
async function init() {
  try {
    ...
    state.listeningLadder = normalizeListeningLadder(
      await getValue("listeningLadder", defaultListeningLadder())
    );
    ...
    state.customNotes = await a1Storage.getAllRecords("vocabulary");
    state.schedules = new Map((await a1Storage.getAllRecords("schedules")).map((item) => [item.id, item]));
    state.reviewLogs = await a1Storage.getAllRecords("reviewLogs");
    state.exerciseAttempts = new Map((await a1Storage.getAllRecords("exercises")).map((item) => [item.id, item]));
    state.recordings = new Map((await a1Storage.getAllRecords("recordings")).map((item) => [item.id, item]));
...
function renderToday() {
    ...
    <div class="dashboard-grid with-top-gap">
      ...
      <aside class="section-band daily-plan">
        ...
        <ol class="example-list">
          ...
          <li>5 минут: один текст в лестнице аудирования.</li>
        ...
        <button class="secondary-button full-button" type="button" data-go-listening>Лестница аудирования</button>
      </aside>
    </div>
    ${renderResources()}`;
  ...
  document.querySelectorAll("[data-go-listening]").forEach((button) => {
    button.addEventListener("click", () => switchView("listening"));
  });
}

function renderListeningLadder() {
  const drills = getListeningLadderDrills();
  const drill = getCurrentListeningLadderDrill(drills);
  if (!drill) {
    app.innerHTML = `
      <section class="empty-state">
        <strong>Аудирование откроется вместе с первым пройденным текстом</strong>
        <p>Заверши урок с заданием на аудирование — затем здесь появится короткая практика без транскрипта до попытки.</p>
      </section>`;
    return;
  }

  const ladder = state.listeningLadder;
  const gistCorrect = ladder.gistAnswer === drill.gist.answer;
  const detailUnlocked = ladder.step !== "gist";
  const dictationUnlocked = ladder.step === "dictation";
  const dictationResult = ladder.dictationResult;
  app.innerHTML = `
    <section class="listening-ladder">
      <header class="lesson-hero listening-ladder-hero">
        <div>
          <p class="eyebrow">Короткая регулярная практика · 5–7 минут</p>
          <h3>Лестница аудирования</h3>
          <p class="lesson-goal">Один знакомый текст: сначала улови ситуацию, затем детали, а в конце восстанови фразу на слух. Транскрипт откроется только после попытки.</p>
        </div>
        <div class="completion-actions">
          <span class="tag amber">${drills.length} открытых записей</span>
          <p class="completion-status">${escapeHtml(drill.lesson.level)} · ${escapeHtml(drill.lesson.title)}</p>
        </div>
      </header>

      <ol class="listening-ladder-progress" aria-label="Шаги лестницы">
        <li class="${gistCorrect ? "complete" : "active"}"><strong>1. Смысл</strong><span>Выбери ситуацию</span></li>
        <li class="${detailUnlocked ? (dictationUnlocked ? "complete" : "active") : "locked"}"><strong>2. Детали</strong><span>Ответь на вопрос</span></li>
        <li class="${dictationUnlocked ? "active" : "locked"}"><strong>3. Диктант</strong><span>Восстанови текст</span></li>
      </ol>

      <section class="exercise listening-ladder-step" aria-labelledby="listening-gist-title">
        <div class="exercise-header"><span class="tag rose">шаг 1</span><span class="result-badge ${gistCorrect ? "correct" : ""}">${gistCorrect ? "готово" : "сначала слушай"}</span></div>
        <h4 id="listening-gist-title">Смысл</h4>
        <p><strong>${escapeHtml(drill.gist.prompt)}</strong></p>
        <div class="control-row"><button class="primary-button" type="button" data-listening-speak>▶ Прослушать</button></div>
        <div class="listening-gist-options">
          ${drill.gist.options.map((option) => `
            <button class="secondary-button ${ladder.gistAnswer === option ? "selected" : ""}" type="button" data-listening-gist="${escapeHtml(option)}">
              ${escapeHtml(option)}
            </button>`).join("")}
        </div>
        ${ladder.gistAnswer ? `<div class="exercise-feedback ${gistCorrect ? "correct" : "incorrect"}">${gistCorrect ? "Верно. Теперь вытащи конкретную информацию." : "Пока не совпало. Послушай ещё раз и попробуй выбрать ситуацию целиком."}</div>` : ""}
      </section>

      <section class="exercise listening-ladder-step ${detailUnlocked ? "" : "locked"}" aria-labelledby="listening-detail-title">
        <div class="exercise-header"><span class="tag rose">шаг 2</span><span class="result-badge ${ladder.detailResult?.status || ""}">${detailUnlocked ? "детали" : "после шага 1"}</span></div>
        <h4 id="listening-detail-title">Детали</h4>
        ${detailUnlocked ? `
          <p><strong>${escapeHtml(drill.detail.prompt)}</strong></p>
          <textarea id="listening-detail-answer" placeholder="Запиши ответ по памяти…">${escapeHtml(ladder.detailAnswer)}</textarea>
          <div class="control-row">
            <button class="pill-button" type="button" data-listening-speak>▶ Послушать ещё раз</button>
            <button class="primary-button" type="button" data-listening-check-detail>Проверить детали</button>
          </div>
          ${ladder.detailResult ? `<div class="exercise-feedback ${escapeHtml(ladder.detailResult.status)}">${escapeHtml(ladder.detailResult.message)}</div>` : ""}
          ${ladder.detailResult && !dictationUnlocked ? `<button class="secondary-button listening-next-step" type="button" data-listening-open-dictation>К диктанту →</button>` : ""}
          ${dictationUnlocked ? `<div class="model-answer"><strong>Ориентир по деталям</strong><p>${escapeHtml(drill.detail.modelAnswer)}</p></div>` : ""}`
          : `<p class="note">Сначала выбери ситуацию без текста.</p>`}
      </section>

      <section class="exercise listening-ladder-step ${dictationUnlocked ? "" : "locked"}" aria-labelledby="listening-dictation-title">
        <div class="exercise-header"><span class="tag rose">шаг 3</span><span class="result-badge ${dictationResult?.status || ""}">${dictationUnlocked ? "диктант" : "после шага 2"}</span></div>
        <h4 id="listening-dictation-title">Диктант</h4>
        ${dictationUnlocked ? `
          <p><strong>Прослушай запись и восстанови весь текст. Не открывай предыдущий ответ.</strong></p>
          <textarea id="listening-dictation-answer" placeholder="Запиши услышанный текст полностью…">${escapeHtml(ladder.dictationAnswer)}</textarea>
          <div class="control-row">
            <button class="pill-button" type="button" data-listening-speak>▶ Прослушать ещё раз</button>
            <button class="primary-button" type="button" data-listening-check-dictation>Проверить диктант</button>
          </div>
          ${dictationResult ? `<div class="exercise-feedback ${escapeHtml(dictationResult.status)}">${escapeHtml(dictationResult.message)}</div>
            <div class="exercise-transcript"><strong>Транскрипт</strong><p>${escapeHtml(drill.transcript)}</p></div>
            <button class="secondary-button listening-next-step" type="button" data-listening-complete>Завершить и выбрать следующую запись →</button>` : ""}`
          : `<p class="note">Сначала ответь на вопрос по деталям.</p>`}
      </section>

      ${renderListeningLadderHistory(ladder.history, drills)}
    </section>`;

  bindListeningLadder(drill);
}

function getListeningLadderDrills() {
  const completedLessons = new Set(getCompletedLessonIds(state.appState.completedLessons));
  return state.data.lessons
    ...
    .filter(({ exercise }) => exercise?.listeningLadder && hasListeningLadderData(exercise.listeningLadder))
    .map(({ lesson, exercise }) => ({
      id: exercise.id,
      lesson,
      exercise,
      audioText: exercise.listenText || exercise.transcript,
      transcript: exercise.transcript,
      gist: exercise.listeningLadder.gist,
      detail: exercise.listeningLadder.detail
    }));
}

function hasListeningLadderData(value) {
  return Boolean(
    value
    && typeof value === "object"
    && value.gist
    && Array.isArray(value.gist.options)
    && value.gist.options.includes(value.gist.answer)
    && value.detail
  );
}

function getCurrentListeningLadderDrill(drills) {
  const current = drills.find((drill) => drill.id === state.listeningLadder.currentDrillId);
  if (current) return current;
  const next = getNextListeningLadderDrill(drills, state.listeningLadder.history);
  if (!next) return null;
  state.listeningLadder = resetListeningLadderForDrill(next.id, state.listeningLadder.history);
  return next;
}

function getNextListeningLadderDrill(drills, history = []) {
  if (!drills.length) return null;
  const lastDrillId = history[0]?.drillId;
  const lastIndex = drills.findIndex((drill) => drill.id === lastDrillId);
  return drills[(lastIndex + 1 + drills.length) % drills.length];
}

function resetListeningLadderForDrill(drillId, history = []) {
  return {
...
function renderListeningLadderHistory(history, drills) {
  if (!history.length) return "";
  const drillLabels = new Map(drills.map((drill) => [drill.id, drill.lesson.title]));
  return `
    <section class="section-band listening-history">
      <div class="section-heading"><div><p class="eyebrow">Последние сессии</p><h4>Регулярность без серии ради серии</h4></div></div>
      <div class="history-list">${history.slice(0, 5).map((entry) => `
        <div class="history-row"><strong>${escapeHtml(drillLabels.get(entry.drillId) || "Пройденная запись")}</strong><span>${escapeHtml(listeningResultLabel(entry.dictationStatus))}</span><time>${formatDateTime(entry.completedAt)}</time></div>`).join("")}</div>
    </section>`;
}

function bindListeningLadder(drill) {
  document.querySelectorAll("[data-listening-speak]").forEach((button) => {
    button.addEventListener("click", () => speakFrench(drill.audioText));
  });
  document.querySelectorAll("[data-listening-gist]").forEach((button) => {
    button.addEventListener("click", async () => {
      state.listeningLadder.gistAnswer = button.dataset.listeningGist;
      if (state.listeningLadder.gistAnswer === drill.gist.answer) state.listeningLadder.step = "detail";
      await saveListeningLadder();
      renderListeningLadder();
    });
  });
  const detailInput = document.querySelector("#listening-detail-answer");
  detailInput?.addEventListener("input", () => {
    state.listeningLadder.detailAnswer = detailInput.value;
    state.listeningLadder.detailResult = null;
    debounceSave("listeningLadder", () => setValue("listeningLadder", state.listeningLadder));
  });
  document.querySelector("[data-listening-check-detail]")?.addEventListener("click", async () => {
    state.listeningLadder.detailAnswer = detailInput?.value || "";
    state.listeningLadder.detailResult = checkListeningLadderDetail(drill, state.listeningLadder.detailAnswer);
    await saveListeningLadder();
    renderListeningLadder();
  });
  document.querySelector("[data-listening-open-dictation]")?.addEventListener("click", async () => {
    state.listeningLadder.step = "dictation";
    await saveListeningLadder();
    renderListeningLadder();
  });
  const dictationInput = document.querySelector("#listening-dictation-answer");
  dictationInput?.addEventListener("input", () => {
    state.listeningLadder.dictationAnswer = dictationInput.value;
    state.listeningLadder.dictationResult = null;
    debounceSave("listeningLadder", () => setValue("listeningLadder", state.listeningLadder));
  });
  document.querySelector("[data-listening-check-dictation]")?.addEventListener("click", async () => {
    state.listeningLadder.dictationAnswer = dictationInput?.value || "";
    state.listeningLadder.dictationResult = checkListeningLadderDictation(drill, state.listeningLadder.dictationAnswer);
    await saveListeningLadder();
    renderListeningLadder();
  });
  document.querySelector("[data-listening-complete]")?.addEventListener("click", async () => {
    const history = [{
      drillId: drill.id,
      completedAt: new Date().toISOString(),
      gistCorrect: state.listeningLadder.gistAnswer === drill.gist.answer,
      detailStatus: state.listeningLadder.detailResult?.status || "incorrect",
      dictationStatus: state.listeningLadder.dictationResult?.status || "incorrect"
    }, ...state.listeningLadder.history].slice(0, 30);
    const next = getNextListeningLadderDrill(getListeningLadderDrills(), history);
    state.listeningLadder = resetListeningLadderForDrill(next?.id || null, history);
    await saveListeningLadder();
    renderListeningLadder();
  });
}

function checkListeningLadderDetail(drill, answer) {
  return checkExercise({
    type: "listening-comprehension",
    acceptedAnswers: drill.detail.acceptedAnswers,
    modelAnswer: drill.detail.modelAnswer,
    hints: []
  }, answer);
}

function checkListeningLadderDictation(drill, answer) {
  const value = String(answer || "").trim();
  if (!value) return { status: "incorrect", message: "Сначала запиши услышанный текст." };
  const normalized = normalizeAnswer(value);
  const expected = normalizeAnswer(drill.transcript);
  if (normalized === expected) return { status: "correct", message: "Текст восстановлен точно. Проверь транскрипт и переходи к следующей записи." };
  if (stripDiacritics(normalized) === stripDiacritics(expected)) {
    return { status: "almost", message: "Слова на месте. Проверь акценты и апострофы по транскрипту." };
  }
  return { status: "incorrect", message: "Не страшно: сравни свою попытку с транскриптом, отметь потерянный фрагмент и повтори запись вслух." };
}

async function saveListeningLadder() {
  return runSave(() => setValue("listeningLadder", state.listeningLadder));
}
...
function renderReview() {
  ...
  queue = applyReviewQueueOrder(queue, state.schedules, state.reviewQueueOrder, state.reviewSeen);
  const card = queue[0];
  ...
  app.innerHTML = `
    <section class="review-stage">
      <div class="review-toolbar">
        ...
        <button class="secondary-button" type="button" id="review-shuffle" ${queue.length ? "" : "disabled"}>Перемешать карточки</button>`}
...
function renderPronunciationReview() {
  ...
  queue = applyReviewQueueOrder(
    queue,
    state.schedules,
    state.pronunciationReviewQueueOrder,
    state.pronunciationReviewSeen
  );
  const card = queue[0];
  ...
  app.innerHTML = `
    <section class="review-stage pronunciation-review-stage">
      <div class="review-toolbar pronunciation-review-toolbar">
        ...
        ${isCatalog ? "" : `<button class="secondary-button" type="button" id="pronunciation-review-shuffle" ${queue.length ? "" : "disabled"}>Перемешать карточки</button>`}
  ...
  bindPronunciationReviewToolbar(queue);
...
function bindPronunciationReviewToolbar(queue) {
  ...
  document.querySelector("#pronunciation-review-shuffle")?.addEventListener("click", () => {
    state.pronunciationReviewQueueOrder = createShuffledQueueOrder(queue, state.schedules);
    state.pronunciationReviewAnswerVisible = false;
    renderPronunciationReview();
  });
    ...
    <div class="settings-layout">
      <section class="section-band">
        ...
        <label class="field-label">Имя
          <input id="learner-name" type="text" maxlength="40" autocomplete="name" placeholder="Например, Анна" value="${escapeHtml(state.settings.learnerName)}" />
        </label>
        <p class="note">Имя появится рядом с переключателем уровня и будет общим для A1 и A2.</p>
      </section>
...
function createShuffledQueueOrder(queue, schedules) {
  const shuffled = shuffleReviewQueueByPool(queue, schedules);
  return {
    review: shuffled.filter((card) => !isNewSchedule(schedules.get(card.id))).map((card) => card.id),
    new: shuffled.filter((card) => isNewSchedule(schedules.get(card.id))).map((card) => card.id)
  };
}

function applyReviewQueueOrder(queue, schedules, queueOrder, seen) {
  if (!queueOrder) return queue;
  const reviewCards = [];
  const newCards = [];
  ...
  return [
    ...orderQueuePool(reviewCards, queueOrder.review, seen),
    ...orderQueuePool(newCards, queueOrder.new, seen)
  ];
}

function orderQueuePool(cards, order, seen) {
  const positions = new Map(order.map((id, index) => [id, index]));
  return cards
    .map((card, index) => ({ card, index }))
    .sort((left, right) => {
      const seenDifference = Number(seen.has(left.card.id)) - Number(seen.has(right.card.id));
      if (seenDifference) return seenDifference;
      const leftPosition = positions.get(left.card.id) ?? order.length + left.index;
      const rightPosition = positions.get(right.card.id) ?? order.length + right.index;
      return leftPosition - rightPosition;
    })
    .map(({ card }) => card);
}

function bindReviewToolbar(queue) {
  ...
  document.querySelector("#review-shuffle")?.addEventListener("click", () => {
    state.reviewQueueOrder = createShuffledQueueOrder(queue, state.schedules);
    state.reviewAnswerVisible = false;
    renderReview();
  });
...
function bindSettingsActions() {
  ...
  learnerName.addEventListener("input", () => {
    state.settings.learnerName = normalizeLearnerName(learnerName.value);
    announceLearnerName(state.settings.learnerName);
    debounceSave("settings", () => setValue("settings", state.settings));
  });
  learnerName.addEventListener("change", () => {
    learnerName.value = state.settings.learnerName;
...
function getActiveCards() {
  ...
  return getAllCards().filter((card) => {
    const unlocked = card.source === "custom" || completedLessons.has(card.lessonId);
    return unlocked
...
function defaultListeningLadder() {
  return {
    currentDrillId: null,
    step: "gist",
    gistAnswer: null,
...
function normalizeListeningLadder(value) {
  const saved = value && typeof value === "object" && !Array.isArray(value) ? value : {};
  const validSteps = new Set(["gist", "detail", "dictation"]);
  const history = Array.isArray(saved.history)
    ? saved.history
      .filter((entry) => entry && typeof entry === "object" && typeof entry.drillId === "string" && isDateString(entry.completedAt))
      .slice(0, 30)
      .map((entry) => ({
        drillId: entry.drillId,
        completedAt: entry.completedAt,
        gistCorrect: entry.gistCorrect === true,
        detailStatus: ["correct", "almost", "incorrect"].includes(entry.detailStatus) ? entry.detailStatus : "incorrect",
        dictationStatus: ["correct", "almost", "incorrect"].includes(entry.dictationStatus) ? entry.dictationStatus : "incorrect"
      }))
    : [];
  return {
    currentDrillId: typeof saved.currentDrillId === "string" ? saved.currentDrillId : null,
    step: validSteps.has(saved.step) ? saved.step : "gist",
    gistAnswer: typeof saved.gistAnswer === "string" ? saved.gistAnswer : null,
    detailAnswer: typeof saved.detailAnswer === "string" ? saved.detailAnswer : "",
    detailResult: normalizeListeningResult(saved.detailResult),
    dictationAnswer: typeof saved.dictationAnswer === "string" ? saved.dictationAnswer : "",
    dictationResult: normalizeListeningResult(saved.dictationResult),
    history
  };
}

function normalizeListeningResult(value) {
  if (!value || typeof value !== "object" || !["correct", "almost", "incorrect"].includes(value.status)) return null;
  return { status: value.status, message: typeof value.message === "string" ? value.message : "" };
}
...
function announceLearnerName(value) {
  window.dispatchEvent(new CustomEvent("french-study:learner-name", { detail: value }));
}
...
function listeningResultLabel(status) {
  return { correct: "диктант точный", almost: "почти точный", incorrect: "нужна сверка" }[status] || "завершено";
}
